...
feature_cols = feature_cols +  [c for c in X_train_df.columns if c.endswith("_mrna")]
feature_cols = feature_cols + [c for c in X_train_df.columns if c.endswith("_methy")]
...
num_col = num_col + [c for c in X_train.columns if c.endswith("_methy")]+ [c for c in X_train.columns if c.endswith("_mrna")]
...
optimizer = tt.optim.Adam(lr=1e-3, weight_decay=5e-2)
...
plt.title(f"Test set: KM by predicted risk group\nLog-rank p={res.p_value:.3e}")
plt.show()


# mRNA columns
mrna_cols = [c for c in X_train_df.columns if c.endswith("_mrna")]

# methylation columns
methy_cols = [c for c in X_train_df.columns if c.endswith("_methy")]

print("Number of mRNA features:", len(mrna_cols))
print("Number of methylation features:", len(methy_cols))
...
print("n_features:", len(feature_names), " X_train cols:", X_train.shape[1])
...
def deeptd_cindex(model, X, durations, events):
    model.compute_baseline_hazards()
    surv = model.predict_surv_df(X)
    ev = EvalSurv(surv, durations, events, censor_surv='km')
    return ev.concordance_td()

def permutation_importance_deepsurv(
    model, X, durations, events, feature_names,
    n_repeats=10, random_state=42, topk=30
):
    rng = np.random.default_rng(random_state)
    X = np.asarray(X).copy()

    base = deeptd_cindex(model, X, durations, events)

    p = X.shape[1]
    drops = np.zeros((p, n_repeats), dtype=float)

    for j in range(p):
        for r in range(n_repeats):
            Xp = X.copy()
            Xp[:, j] = rng.permutation(Xp[:, j])
            drops[j, r] = base - deeptd_cindex(model, Xp, durations, events)

    imp_mean = drops.mean(axis=1)
    imp_sd   = drops.std(axis=1, ddof=1)

    idx = np.argsort(imp_mean)[::-1][:topk]
    return base, idx, imp_mean[idx], imp_sd[idx], np.array(feature_names)[idx]

# ---- run on validation set ----
base_c, idx, m, s, names = permutation_importance_deepsurv(
    model,
    X_val, dur_val, ev_val,
    feature_names,
    n_repeats=10,        # 先用10，稳定后可提高到30-50
    random_state=42,
    topk=30
)

print("Baseline Val C-index:", base_c)

plt.figure(figsize=(8, 10))
plt.barh(names[::-1], m[::-1])
plt.xlabel("Importance")
plt.title("DeepSurv permutation importance (Top 30 on Val)")
plt.tight_layout()
plt.show()